...
export default function index({ google_map_settings }) {
    ...
    const toggleStatus = (id) => {
        put(route('dashboard.settings.google-map-settings.toggle-status', id));
    };

    const [columns, setColumns] = useState([]);
    useEffect(() => {
        const columns = [
            { key: 'google_map_api_key', label: 'Google Map API Key' },
            { key: 'google_map_id', label: 'Google Map ID' },

            {
                label: 'Google Map Setting Status',
                render: (item) => {
                    if (item.is_active === 1) {
                        return (
                            <>
                        ...
                        <>
                            <div className="my-3 flex flex-wrap justify-end gap-4">
                                <LinkButton
                                    ...
                                    URL={route('dashboard.settings.google-map-settings.create')}
                                    Icon={
                                        <svg
                            ...
                            <Table
                                ...
                                BulkDeleteRoute={
                                    'dashboard.settings.google-map-settings.destroybyselection'
                                }
                                SingleDeleteRoute={'dashboard.settings.google-map-settings.destroy'}
                                EditRoute={'dashboard.settings.google-map-settings.edit'}
                                SearchRoute={'dashboard.settings.google-map-settings.index'}
                                Search={false}
                                DefaultSearchInput={false}
                                items={google_map_settings}
                                props={props}
                                columns={columns}
                            />
                        </>
